...
Java_com_colorflow_music_BeatDetector_detect(JNIEnv *env, jobject instance) {
    uint_t samplerate = 0;
    ...
    do {
        ...
        if (out->data[0] != 0) {
            __android_log_print(ANDROID_LOG_INFO, APPNAME, "beat at %.3fms, %.3fs, frame %d, %.2fbpm with confidence %.2f\n",
    ...
    __android_log_print(ANDROID_LOG_INFO, APPNAME, "read %.2fs, %d frames at %dHz (%d blocks) from %s\n",
