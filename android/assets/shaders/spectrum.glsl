...
void main()
{
    //gl_FragColor = vec4(texture2D(iChannel0, v_texCoords).xyz, 1.0);
    //return;
    ...
    float sm = 1.0 / resolution.x * 2.0; // smooth
    float ps = 1.0 / resolution.x * sqrt(resolution.x) * 2.25; // circle thin
    ...
    for(float i = CIRCLES; i > 1.0; i -= 1.0) {
        ...
        if (subm > 0.0) {
            float yolo = i / CIRCLES * 0.5 + time * 0.05 + mP0 * 0.84;
            col = clamp(abs(fract(yolo + vec4(3, 2, 1, 0) / 3.0) * 6.0 - 3.0) - 1.0 , 0.0, 1.0).rgb;
        }
